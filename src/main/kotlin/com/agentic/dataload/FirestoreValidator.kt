...
object FirestoreValidator {
    ...
    fun validateQuestionsCollection(collectionName: String) {
        val db: Firestore = FirestoreClient.getFirestore()
        val allCollections = db.listCollections().map { it.id }
        var usedCollectionName = collectionName
        var metaCollectionName = collectionName+ "_Class7"
        ...
        val prefixMatches = allCollections.filter { it != usedCollectionName && (it.startsWith(collectionName + "_") || it.startsWith(collectionName + "-")) }
        ...
        if (expectedCount != null) {
            if (expectedCount == actualCount) {
                ...
                countSummaryMsg = "WARNING: Question count ($actualCount) does not match metadata.$collectionName.count ($expectedCount)"
            }
        } else {
            countSummaryMsg = "No metadata count found for collection '$collectionName'."
            if (prefixMatches.isNotEmpty()) {
                countSummaryMsg += "\nCollections with similar names found (${prefixMatches.size}): ${prefixMatches.joinToString(", ")}" 
            }
        }
        ...
        val questionDocMap = mutableMapOf<String, MutableList<Pair<String, Int>>>() // question -> list of (docId, qno)
        ...
        for (doc in docs) {
            ...
            // Track question to docId/qno
            questionDocMap.getOrPut(question) { mutableListOf() }.add(Pair(doc.id, qno))
        ...
        // --- Summary reporting ---
        // Count duplicate questions (by text)
        ...
        if (duplicateQuestions.isNotEmpty()) {
            println("Duplicate question texts:")
            duplicateQuestions.forEach { (text, count) ->
                println("'$text'")
                // List all doc ids and qnos for this question
                questionDocMap[text]?.forEach { (docId, qno) ->
